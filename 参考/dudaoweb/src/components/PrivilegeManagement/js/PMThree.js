  ...
  methods: {
    ...
    getRolePerson () {
      this.$ajax({
        ...
        data: {roleType: 1, roleName: this.currRole.roleName} // 1：教育局
      }).then(res => {
        let that = this
    ...
    getAllPerson () {
      this.$ajax({
        method: 'get',
        url: 'baseInfo/roleController/areaInfo'
      }).then(res => {
        this.allPersonList.splice(0, this.allPersonList.length) // 清空数组
        // 处理数据 => 树形结构（此处只有两级）
        this.allPersonList = transTree(Object.assign([], res.data.data.areaList))
      })
    },
    changeRole (curr) {
