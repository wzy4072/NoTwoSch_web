<template>
    <div style="padding:30px 50px;" id="goods-pay">
        <v-spin :spinning="spinning">
            <v-card title="支付" v-if="!paySuccess">
                <h3>订单信息</h3>
            ...
            <v-card title="支付成功" v-else>
                <p style="font-size: 18px;text-align: center">支付成功，如果您的浏览器没有自动关闭页面，请手动关闭本页面</p>
            </v-card>
        </v-spin>
        <v-modal title="扫码支付" 
            ...
            <div class="weixinQRcode" v-show="!paySuccess">
                <h5>支付金额</h5>
...
export default {
    ...
    created() {
        ...
        this.spinning = true;
        api.getOrderView({ "code": options.code, "variationCode": options.variationCode, "qty": options.qty, "behavior": options.behavior, "currency": options.currency, "type": options.type })
            .then(resp => {
                this.spinning = false;
                if (resp.success) {
                    ...
                    this.orderInfo.corporateId = options.corporateId;
                    this.orderInfo.qty = options.qty;
...
.weixinQRcode {
  text-align: center;
  height: 485px;
  background: url("/static/images/goods/payBackground.png") bottom center
    no-repeat;
}
.weixinQRcode h5 {
  font-size: 16px;
