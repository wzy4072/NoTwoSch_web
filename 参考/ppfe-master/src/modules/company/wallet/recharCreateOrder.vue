                    ...
                    <div class="amout-form">
                        <v-form direction="horizontal" :model="amountList" :rules="amountRules" ref="amountList">
                            ...
                            <div class="radio-group">
                                <div 
                                v-for="(val,index) in valList"
                                :key="index"
                                 class="radio-btn"
                                 :class="{select:amountList.price==val}"
                                 @click="amountList.price = val">{{val}}</div>
                                <div class="radio-btn" :class="{select:getClass}" @click="elseNum">其他金额</div>
                                <div class="elsenum" v-show="showIpt"><span>输入金额：</span><input type="text" v-model="amountList.price"></div>
                            </div>
                            <button type="button" @click="subOrder('amountList')" class="_btn" style="height: 48px;">立即支付</button>
                        </v-form>
                    </div>
                </template>
            </recharge>
    ...
    <script>
        ...
        export default {
            ...
            data() {
                return {
                    ...
                    amountList: {
                        ...
                        walletId: null,
                        payType: 'WX',
                        tradeType: 'NATIVE', // 微信的固定搭配 
                        orderType: 'RECHARGE',//RECHARGE   PAYMENT 充值 余额付款 充值付款
            ...
            computed: {
                ...
                getClass() {
                    ...
                    for (let i = 0; i < _this.valList.length; i++) {
                        if (_this.valList[i] == _this.amountList.price) { flag = false; }
                    };
                    return flag;
                }
            },
            methods: {
                subOrder(formName) {
                    let _this = this;
                    _this.amountList.walletId = _this.loginContext.currentCorporate.walletId;
                    if(!/^[0-9]*(\.[0-9]{1,2})?$/.test(_this.amountList.price)){ _this.$message.info('充值金额不合法,请重新输入！' ); return false;}
                    payapi.createChargeOrder(_this.amountList)
                        .then(req => {
                            if(req.success){
